$("#orders-middle-toolTip").tooltip();

$(window).on("scroll", function() {
	var scrollNow = window.scrollY;

    if(scrollNow > 900) {
        $(".orders-wrp-right").css('position', 'absolute');
        $(".orders-wrp-right").css('top', '67%');
    } else {
       $(".orders-wrp-right").css('position', 'fixed');
       $(".orders-wrp-right").css('top', '43%');
    }
});
